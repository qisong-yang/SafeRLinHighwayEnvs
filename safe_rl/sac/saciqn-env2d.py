#!/usr/bin/env python
#env1: 3 dynamic hazards and 1 dynamic target 
#11.13 revove stop_gradient for qc and qc_pi
...
def get_tau(batch_size, num_quantiles, tau_type = 'iqn'):
    """
...
def _quantile_regression_loss(_input, target, tau, weight):
    ...
    #L = F.smooth_l1_loss(expanded_input, expanded_target, reduction="none")
    #L = Smooth_l1_loss(_input, target)  # (N, T, T)
...
def dist_critic(x, a, pi, tau_hat, tau, name, hidden_sizes=(64,64),activation=tf.nn.relu):
...
'''
def dist_critic(x, a, pi, tau, name, _reuse = False, hidden_sizes=(256,256), activation=tf.nn.relu,
               output_activation=None, policy=mlp_gaussian_policy, action_space=None):
    ...
    with tf.variable_scope(name, reuse = _reuse):
        z_pred = fn_mlp(tf.concat([x, a, tau], axis=-1))

    with tf.variable_scope(name, reuse=True):
        z_pi = fn_mlp(tf.concat([x, pi, tau], axis=-1))
...
Soft Actor-Critic
"""
def sac(actor_fn=mlp_actor, critic_fn=mlp_critic, zdist_fn = dist_critic, quantile_regression_loss = _quantile_regression_loss, ac_kwargs=dict(), seed=0, num_quantiles=20, steps_per_epoch=1000, epochs=100, replay_size=int(1e6), gamma=0.99,
        ...
        fixed_cost_penalty=None, cost_constraint=None, cost_lim1=None, cost_lim2=None,
        reward_scale=1,
        ):
    """
    ...
    """
    use_costs = fixed_cost_penalty or cost_constraint or cost_lim1
    #use_costs = False
    ...
    _cost_lim = tf.placeholder(dtype=tf.float32)
    
    
    local_batch_size = batch_size // num_procs()
    '''
    tau = np.ones([local_batch_size, 1]) * cl
    ...
    with tf.variable_scope('main'):
        ...
        #qz1_pred, qz1t_pred, _, qz1t_pi = zdist_fn(x_ph, a_ph, pi, tau_hat_ph, tau, name='qz1', **ac_kwargs)
        #qz2_pred, qz2t_pred, _, qz2t_pi = zdist_fn(x_ph, a_ph, pi, tau_hat_ph, tau, name='qz2', **ac_kwargs)
    ...
    with tf.variable_scope('target'):
        ...
        #_, _, qz1_pi_targ, _  = zdist_fn(x2_ph, a_ph, pi2, next_tau_hat_ph, tau, name='qz1', **ac_kwargs)
        #_, _, qz2_pi_targ, _  = zdist_fn(x2_ph, a_ph, pi2, next_tau_hat_ph, tau, name='qz2', **ac_kwargs)
    ...
    replay_buffer_trans = ReplayBuffer(obs_dim=obs_dim, act_dim=act_dim, size=replay_size)
    ...
    '''avoid None value'''
    '''
    qz1_pred = tf.clip_by_value(qz1_pred, 1e-8, 1e8)
    qz2_pred = tf.clip_by_value(qz2_pred, 1e-8, 1e8)
    
    qz1t_pred = tf.clip_by_value(qz1t_pred, 1e-8, 1e8)
    qz2t_pred = tf.clip_by_value(qz2t_pred, 1e-8, 1e8)
    
    qz1t_pi = tf.clip_by_value(qz1t_pi, 1e-8, 1e8)
    qz2t_pi = tf.clip_by_value(qz2t_pi, 1e-8, 1e8)
    
    qz1_pi_targ = tf.clip_by_value(qz1_pi_targ, 1e-8, 1e8)
    qz2_pi_targ = tf.clip_by_value(qz2_pi_targ, 1e-8, 1e8)
    '''
    ...
    #qzt_pred = tf.maximum(qz1t_pred, qz2t_pred)
    #qzt_pi = tf.maximum(qz1t_pi, qz2t_pi)
    #qz_pi_targ = tf.maximum(qz1_pi_targ, qz2_pi_targ)
    ...
    #qzf1_loss = quantile_regression_loss(qz1_pred, qz_target, tau_hat_ph, next_presum_tau_ph)
    #qzf2_loss = quantile_regression_loss(qz2_pred, qz_target, tau_hat_ph, next_presum_tau_ph)
    ...
    if use_costs:
        if cost_constraint is None:
            ...
            cost_constraint = _cost_lim * (1 - gamma ** 150) / (1 - gamma) / 150
    ...
    if use_costs and fixed_cost_penalty is None:
        ...
        #costpen_optimizer = MpiAdamOptimizer(learning_rate=lr*120)
        #costpen_optimizer = MpiAdamOptimizer(learning_rate=lr*200)
        #costpen_optimizer = MpiAdamOptimizer(learning_rate=lr*250)
        if fixed_entropy_bonus is None:
    ...
    for t in range(total_steps // num_procs()):
        ...
        if epoch <= epochs/2 :
            ...
            replay_buffer_trans.store(o, a, r, o2, d, c)
        ...
        if t > 0 and t % update_freq == 0:
            ...
            for j in range(update_freq):
                ...
                if epoch <= epochs/2 :
                    ...
                    batch = replay_buffer_trans.sample_batch(local_batch_size)
                
                _, tau_hat, _ = get_tau(local_batch_size, num_quantiles, tau_type = 'iqn')
                _, next_tau_hat, next_presum_tau = get_tau(local_batch_size, num_quantiles, tau_type = 'iqn')
                _, tau, presum_tau = get_tau(local_batch_size, num_quantiles, tau_type = 'iqn')
        ...
        if t > 0 and t % local_steps_per_epoch == 0:
            ...
            #logger.log_tabular('Z2TP', with_min_and_max=True)
            ...
            #logger.log_tabular('LossZF2', average_only=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cost_lim1', type=float, default=None)
    parser.add_argument('--cost_lim2', type=float, default=None)
    ...
    sac(actor_fn=mlp_actor, critic_fn=mlp_critic, zdist_fn = dist_critic, quantile_regression_loss = _quantile_regression_loss, ac_kwargs=dict(hidden_sizes=[args.hid]*args.l),
        gamma=args.gamma, seed=args.seed, num_quantiles=args.nq, epochs=args.epochs, batch_size=args.batch_size,
        ...
        fixed_cost_penalty=args.fixed_cost_penalty, cost_constraint=args.cost_constraint, cost_lim1 = args.cost_lim1, cost_lim2 = args.cost_lim2
        )
