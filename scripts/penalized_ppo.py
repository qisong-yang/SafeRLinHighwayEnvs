...
if __name__ == '__main__':
    ...
    def launch_ppo_lagrangian(seed, env_name):
        ppo_lagrangian(
            env_fn = lambda : gym.make(env_name, observation=args.observation),
            ac_kwargs = dict(hidden_sizes=(64,64)),
            logger_kwargs = {"output_dir": f"./results/ppo_lag_{env_name}_cost_lim_{args.cost_lim}_penalty_init_{args.penalty_init}_penalty_lr_{args.penalty_lr}_penalty_iters_{args.penalty_iters}_seed_{seed}"},
            render=False,
            max_ep_len=100,
            epochs=150,
    ...
    with Pool() as pool:
        pool.starmap(launch_ppo_lagrangian, product(seeds, env_names))
